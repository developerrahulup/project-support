...
for FOLDER in $BASE_DIR; do
  ...
  # Change to the directory containing the files and verify the GPG signature
  cd "$TG_SOLUTIONS_DIR" && gpg --verify "$SIG_FILE" "$TAR_FILE"
  
  # Check for GPG verification result
  if [ $? -eq 0 ]; then
    echo "GPG signature verification succeeded for $TAR_FILE in $FOLDER."
  else
    echo "GPG signature verification failed for $TAR_FILE in $FOLDER!" >> "$MISSING_CHECKSUM_LOG"
    continue  # Skip checksum validation if GPG verification fails
  fi
